...
class ShizukuEventSource(
    ...
    override suspend fun captureSnapshot(): AppOpsSnapshot {
        ...
        for (cmd in commands) {
            ...
            if (res.exitCode == 0 && res.stdout.isNotBlank()) {
                stdout = res.stdout
                success = true
                Log.d(TAG, "Successfully executed '$cmd' via Shizuku, output size: ${stdout.length} chars")
                break
            } else {
                stderr = res.stderr
                Log.w(TAG, "Shizuku command '$cmd' failed (exit=${res.exitCode}): $stderr")
            }
        }

        if (!success) {
            Log.e(TAG, "All Shizuku appops dump commands failed. Last error: $stderr")
            return AppOpsSnapshot(
                capturedAt = System.currentTimeMillis(),
                entries = emptyList(),
                rawText = "Failed to dump appops via Shizuku (stderr: $stderr)"
            )
        }

        val snapshot = parser.parse(stdout, System.currentTimeMillis())
        Log.d(TAG, "Parsed ${snapshot.entries.size} entries from Shizuku output")
        return snapshot
    }
    ...
    companion object {
        private const val TAG = "ShizukuEventSource"
    }
}
